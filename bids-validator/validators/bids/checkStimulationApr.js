...
const checkStimulationApr = fileList => {
  ...
  for (let key of fileKeys) {
    ...
    if (fileList[key].name.endsWith('.apr')) {
      ...
      stimulationFiles.push(
        fileList[key].name.replace('stimulation.tsv', 'eeg'),
      ) // needed to be changed into eeg to compare to other files
    }
  }

  if (!utils.array.equals(stimulationFiles, aprFiles)) {
    const evidence = constructMissingAprStimEvidence(stimulationFiles, aprFiles)
    issues.push(
      new Issue({
...
const constructMissingAprStimEvidence = (stimulationFiles, aprFiles) => {
  ...
  if (aprNotInStimulationFiles[0] !== '' && aprNotInStimFiles[0] !== '') {
    const evidence =
      'For the following stimulation files there is no corresponding apr file in the dataset:\n                                  ' +
      aprNotInStimulationFiles.join(
        '.tsv \n                                  ',
      ) +
